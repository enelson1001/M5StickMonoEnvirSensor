...
namespace redstone
{
    ...
    void MenuPane::create(int width, int height)
    {
        ...
        // create next (->) button
        gui_buttons[Button35]->create(menu_pane_container);
        gui_buttons[Button35]->align(NULL, LV_ALIGN_CENTER, -10, -2);

        // add button locations on screen
        add_screen_location_of_button(Button35, gui_buttons[Button35]->get_coords());
    }
